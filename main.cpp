// #define GLM_FORCE_DEFAULT_ALIGNED_GENTYPES // makes it so we dont have to
...
class HelloTriangleApplication {
  ...
  private:
    ...
    const std::vector<const char *> validationLayers = {"VK_LAYER_KHRONOS_validation", "VK_LAYER_KHRONOS_validation"};
    ...
    std::vector<VkImage> swapChainImages; // these images are automatically
                                          // cleaned by the swapchain
    ...
    void initVulkan() {
        ...
        std::cout << "created depth buffer resources\n";
        createFrameBuffers();
        std::cout << "created frame buffer" << std::endl;
        createTextureImage();
        std::cout << "created image texture\n"; // we will use command buffers so call
                                                // after createing the command pool
    ...
    void createColorResources() {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL,
                    ...
                    colorImageMemory); // msaaSamples is set during pickPhysical device, so it should be set by the time this function is called
    ...
    void createTextureImage() {
        ...
        // calculates the amount of images that it is possible to use from an image
        // the max selects the largest dimension the log2 figures out how many times it can be divided by two and the floor handles
        // when the image is not a power of 2 and one is added to make it so that the original image has its own mip level
    ...
    void generateMipmaps(VkImage image, VkFormat imageFormat, uint32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
        ...
        // generating mipmaps on the fly is not very common, they are usually stored, so i could do that to get smart
    ...
    void createDescriptorPool() {
        ...
        // be especially carefull here because inadequate pool sizes will not be caught by validation layers
    ...
    void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer, VkDeviceMemory &bufferMemory) {
        ...
        // makes it so that the driver will imediatly copy the data that we tell it
        // to, becase sometimes it wont for things like caching or things like that,
        // and it is also possible that rwites to memory are not visable in the
        // mapped memory imdiatly,this makes the gpu memory coherient this can be
        // less performant that explicitly flushing the memory but it is easier and
        // vktutorial has a reason for it
        ...
        // in a realworld application you are not supposed to call vkallocatememory
        // for every buffer but instead use a custom allocator that a single
        // allocation among many objects using the offset parameters
    ...
    // gpus can have multiple types of memory, differing in allowed operations and
    // performance so we use a function to get the best one
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        clearValues[1].depthStencil = {1.0f, 0}; // make sure that these are in the same order as the attachment
        ...
        VkViewport viewport{}; // we specified that the viewport and scissor was
                               // dynamic so we have to set it here
        viewport.x = 0.0f;
        viewport.y = 0.0f;
    ...
    void createRenderPass() {
        ...
        std::array<VkAttachmentDescription, 3> attachments = {
            ...
            colorAttachmentResolve}; // be carefull the order of these, it has to match in the createframe buffer function
    ...
    void createGraphicsPipeline() {
        ...
        // shader modules are a very thin rapper so we can destroy them when we
        // finish creating the graphics pipeline since they are compiled into native
    ...
    void createSwapChain() {
        ...
        uint32_t imageCount = swapChainSupport.capibilities.minImageCount + 1; // do one more that the minimum because sometimes if we stick with
                                                                               // the minimum we sometimes will have to wait for the driver to
                                                                               // complete internal opterations
        ...
        // if the graphics queue if different from the presentation queue then tell
        // them to share otherwise let them be
        if (indicies.graphicsFamily != indicies.presentFamily) {
            createInfo.imageSharingMode = VK_SHARING_MODE_CONCURRENT; // images can be used in any queue, worse
                                                                      // performance
            createInfo.queueFamilyIndexCount = 2;
            createInfo.pQueueFamilyIndices = queueFamilyIndices; // the queues and amount of queues that the images
                                                                 // are shared  between
        } else {
            createInfo.imageSharingMode = VK_SHARING_MODE_EXCLUSIVE; // ownership of an image has to be expicily
                                                                     // transfered, better performance
            createInfo.queueFamilyIndexCount = 0;
            createInfo.pQueueFamilyIndices = nullptr;
        }

        // swapchain images can be transformed if the gpu supports it so this is
        // saying apply the current transform
        createInfo.preTransform = swapChainSupport.capibilities.currentTransform;
        // this bit if for if you want the alfa channel to blend with other windows
        // in the windowing system or not this means not
        createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
        createInfo.presentMode = presentMode;
        // this says that we dont care about pixels not visable, if you want to read
        // them then set this to false
    ...
    int rateDeviceSuitabality(VkPhysicalDevice device) {
        ...
        // example of something that you could do, this is fully working so use it
        // if you need score +=
        ...
        if (!indicies.isComplete()) {
            std::cout << "no complete indicies\n";
            return 0;
        }
        if (!checkDeviceExtensionSupport(device)) {
            std::cout << "does not support required extentions\n";
            return 0;
        }
        if (!swapChainAdequate) {
            std::cout << "swapchain is not good enough\n";
            return 0;
        }
    ...
    QueueFamilyIndicies findQueueFamilies(VkPhysicalDevice device) {
        ...
        int i = 0;
        // TODO: add logic to make the device selector prefer a device with
        // presentation and drawing in the same queue
    ...
    // what does is it gets the amount of validation layers available and then
    // creates a vector of vkLayerProperties and complares the names of then
    // agains all of the layer names in the validation layers vector which is a
    // list of all of the layers that we want to use
    ...
    void mainLoop() {
        ...
        // because the operation for drawing the frame are asyncronous we need to
        // wait for the to finish before cleaning up
        vkDeviceWaitIdle(logicalDevice);
    }
